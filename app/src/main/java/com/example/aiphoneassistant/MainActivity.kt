...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED ||
            checkSelfPermission(Manifest.permission.READ_CONTACTS) != PackageManager.PERMISSION_GRANTED) requestPermissions(permissions.toTypedArray(), 21)
        home()
    }
    ...
    private fun listenForConversation(){
        ...
        startVoice{spoken-> animateListening(false); val command=stripJarvis(spoken)
    ...
    private fun stripJarvis(spoken:String):String?{val n=spoken.trim().replace(Regex("\\s+")," ");val m=Regex("(?i)\\bjarvis\\b(?:[,:.!-]\\s*)?(.*)").find(n)?:return null;return m.groupValues.getOrNull(1)?.trim()? : ""}
    ...
    private fun callPerson(text:String){
        ...
        val raw=text.lowercase().replace(Regex("^(call|phone|ring)\\s+"),"").trim(); val aliases=when(raw){"mom","my mom","mother","mummy","mama"->listOf("mom","mother","mummy","mama");"dad","my dad","father","daddy","papa"->listOf("dad","father","daddy","papa");else->listOf(raw)}
        val resolver=contentResolver;val projection=arrayOf(ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME,ContactsContract.CommonDataKinds.Phone.NUMBER)
        var name:String?=null;var number:String?=null
        resolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI,projection,null,null,ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME+" ASC")?.use{c->while(c.moveToNext()){val n=c.getString(0);val low=n.lowercase();if(aliases.any{a->low==a||low.contains(a)}){name=n;number=c.getString(1);break}}}
        if(number==null){say("I couldn't find $raw in your contacts.");return}
        say("Calling ${name}.");startActivity(Intent(Intent.ACTION_DIAL, Uri.parse("tel:${Uri.encode(number)}")))
    }

    private fun listenForTrainingControl(){say("Say Jarvis, start training, or Jarvis, stop training.");startVoice{spoken->val c=stripJarvis(spoken);if(c==null){say("Please say Jarvis first.");return@startVoice};when{c.lowercase().contains("start training")->startTrainingFromVoice();c.lowercase().contains("stop training")||c.lowercase().contains("finish training")->stopTrainingFromVoice();else->say("Say Jarvis, start training, or Jarvis, stop training.")}}}
    private fun startTrainingFromVoice(){val s=AssistantAccessibilityService.instance;if(s==null){say("Turn on Accessibility first. I'll open the setting.");startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS));return};s.startTraining();say("Training is on. Demonstrate the actions you want me to learn. Every supported action is saved immediately.");stopTrainingScreen()}
    ...
    private fun training(){val r=root();r.addView(title("🧠 Training Mode"));addAnimatedOrb(r);r.addView(info("Teach Jarvis by demonstration. Every supported action is saved immediately."));val draft=store.loadTrainingDraft();if(draft.isNotEmpty())r.addView(button("♻ RECOVER ${draft.size} SAVED ACTIONS"){reviewRecording(draft)});r.addView(button("🎙 START BY VOICE"){assistant()});r.addView(button("▶ START DEMONSTRATION"){val s=AssistantAccessibilityService.instance;if(s==null){say("Please turn on Accessibility first.");startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))}else{s.startTraining();say("Training started. Perform the actions you want Jarvis to learn.");stopTrainingScreen()}});skills.forEach{s->r.addView(info("• ${s.name} — ${s.steps.size} step(s) — approved=${s.approved}"));r.addView(button("Delete ${s.name}"){store.save(skills.filter{it.id!=s.id});training()})};r.addView(button("← Back"){home()});setContentView(r)}
    private fun stopTrainingScreen(){val r=root();r.addView(title("🧠 Recording"));addAnimatedOrb(r);r.addView(info("Perform actions outside this screen. Return when finished."));r.addView(button("⏹ STOP & REVIEW"){reviewRecording(AssistantAccessibilityService.instance?.stopTraining()?:store.loadTrainingDraft())});r.addView(button("✖ CANCEL"){AssistantAccessibilityService.instance?.clearTrainingDraft();home()});setContentView(r)}
    private fun reviewRecording(steps:List<ActionStep>){val r=root();r.addView(title("Review Training"));r.addView(info("Recorded ${steps.size} supported action(s)."));steps.forEachIndexed{i,s->r.addView(info("${i+1}. ${describe(s)}"))};val name=EditText(this).apply{hint="Skill name"};r.addView(name);r.addView(button("SAVE SKILL"){if(name.text.toString().trim().isEmpty()||steps.isEmpty())say("Give the skill a name and record at least one action.")else{val l=skills;l.add(Skill(System.currentTimeMillis(),name.text.toString().trim(),steps.toMutableList(),false));store.save(l);store.clearTrainingDraft();say("Saved permanently. Approve it in Test Mode.");home()}});r.addView(button("RECORD AGAIN"){training()});r.addView(button("CANCEL"){store.clearTrainingDraft();home()});setContentView(r)}
    private fun testing(){val r=root();r.addView(title("🧪 Test Mode"));r.addView(info("Review, test and approve trained skills."));if(skills.isEmpty())r.addView(info("No trained skills."));skills.forEach{s->r.addView(button("Review: ${s.name}"){reviewSkill(s)})};r.addView(button("← Back"){home()});setContentView(r)}
