...
class MainActivity : Activity() {
    ...
    private fun home() {
        ...
        r.addView(info("V4 • Voice-only commands + persistent training + AI + app discovery"))
        r.addView(info(if (ownerGate.isEnrolled()) "🔐 Owner voice phrase: ENABLED" else "🔐 Owner voice phrase: NOT SET"))
        r.addView(button("🎙 VOICE ASSISTANT"){assistant()})
    ...
    private fun ownerVoiceSetup() {
        ...
        r.addView(info("Speak a private phrase. The assistant will only accept commands that begin with that phrase. Example: say 'Gabriel assistant' during setup, then say 'Gabriel assistant open YouTube'."))
        r.addView(button("🎙 SPEAK OWNER PHRASE") {
            startVoice { spoken ->
                try {
                    ownerGate.enroll(spoken)
                    toast("Owner phrase saved.")
                    home()
                } catch (e: IllegalArgumentException) { toast(e.message ?: "Phrase is too short.") }
            }
        })
        if (ownerGate.isEnrolled()) r.addView(button("REMOVE OWNER PHRASE"){ownerGate.clear();toast("Owner phrase removed.");home()})
    ...
    private fun assistant() {
        val r=root()
        r.addView(title("🎙 Voice-Only Assistant"))
        if (!ownerGate.isEnrolled()) {
            r.addView(info("🔐 Set up your owner voice phrase first. Typed commands are disabled."))
            r.addView(button("SET UP OWNER VOICE"){ownerVoiceSetup()})
        } else {
            r.addView(info("🔒 Voice-only mode. Say your private owner phrase first, then your command. Typed commands are disabled."))
            r.addView(button("🎙 LISTEN FOR COMMAND"){startVoice { spoken ->
                if (!ownerGate.accepts(spoken)) {
                    toast("Voice not accepted. Start with your owner phrase.")
                } else {
                    val command = ownerGate.stripPhrase(spoken)
                    if (command.isBlank()) toast("Now say the command after your owner phrase.") else handleCommand(command)
                }
            }})
        }
        r.addView(button("← Back"){home()})
    ...
    private fun startVoice(result:(String)->Unit){
        voice?.destroy()
        voice=VoiceController(this,{runOnUiThread{result(it)}},{runOnUiThread{toast(it)}})
        voice?.start()
    }
    ...
    private fun handleCommand(text:String){
        when(val c=nlp.parse(text,skills)){
            is ParsedCommand -> when(c.type){
                CommandType.OPEN_APP->{val app=AppDiscovery.findApp(this,c.target); if(app==null)toast("App not found: ${c.target}") else packageManager.getLaunchIntentForPackage(app.packageName)?.let{startActivity(it)}}
                CommandType.RUN_SKILL->{val s=c.skill!!; if(!s.approved)toast("Skill is not approved. Review it in Test Mode.") else execute(s)}
                CommandType.UNKNOWN->toast("I don't understand that command yet.")
            }
        }
    }
    ...
    private fun training(){
        ...
        r.addView(info("Training is now saved after every recorded click, so an app/process restart will not erase the demonstration."))
        ...
        r.addView(button("▶ START DEMONSTRATION"){
            ...
            if(s==null){toast("Enable Accessibility first.");startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))}
            else{s.startTraining();toast("Training started. Perform your actions, then return here.");stopTrainingScreen()}
        })
    ...
    private fun stopTrainingScreen(){
        ...
        r.addView(info("Perform your actions outside this screen. Every supported click is being saved immediately."))
    ...
    private fun reviewRecording(steps:List<ActionStep>){
        val r=root();r.addView(title("Review Training"));r.addView(info("Recorded ${steps.size} supported click action(s)."))
        ...
        r.addView(button("SAVE SKILL"){
            if(name.text.toString().trim().isEmpty()||steps.isEmpty()) toast("Name the skill and record at least one action.")
            else { val l=skills; l.add(Skill(System.currentTimeMillis(),name.text.toString().trim(),steps.toMutableList(),false)); store.save(l);store.clearTrainingDraft();toast("Saved permanently. Review and approve it in Test Mode.");home() }
        })
